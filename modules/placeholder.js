...
function reset() {
  ...
  w = canvas.width = Math.min(window.innerWidth * 0.675, window.innerHeight * 0.9);
  h = canvas.height = w; // Make it square
...
function initParticles() {
  particles = [];
  let numberOfParticles = w * h / 400; // Increased density
...
function drawText(callback) {
  ...
  logo.onload = () => {
    let logoSize = 200;
    let leftMargin = (w - logoSize) / 2;
    let topMargin = (h - logoSize) / 2;
    ctx.drawImage(logo, leftMargin, topMargin, logoSize, logoSize); 
